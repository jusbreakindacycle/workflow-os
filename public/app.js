...
reviewForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const roleCount = graph.phase3 ? new Set(graph.phase3.specs.map((spec) => spec.logical_role)).size : 0;
    document.querySelector('#bootstrap-summary').textContent = graph.phase3
      ? `${graph.nodes.length} strategy-specific WorkItems generated by ${graph.phase3.plannerVersion}; ${roleCount} logical roles were activated by actual capability requirements. Project Pack v${pack.version} generated.`
      : `${graph.nodes.length} compatibility WorkItems generated. Project Pack v${pack.version} generated.`;
