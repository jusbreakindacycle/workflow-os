...
test('local app serves control-plane UI and Phase 4.1 database-backed health endpoint', async (t) => {
  ...
  assert.equal(health.phase, 'phase-4.1');
  assert.equal(health.gate, 'governed-source-control-adapter-implemented');
  assert.equal(health.database.status, 'ready');
  assert.equal(health.database.migrations, 16);
