...
const bridge = await startAntigravityBridge();
...
try {
  ...
  const antigravityRoute = bestWorkerRoute(stateAfterExecution.routes, 'google-antigravity');
  const independentRoute = bestWorkerRoute(stateAfterExecution.routes, hasOpenRouter ? 'openrouter' : 'groq');
  if (!antigravityRoute || !independentRoute) fail('Could not select two independent real routes for portability certification.');
...
function createSyntheticProject({ store, phase1, workspaceId, title }) {
  let snapshot = store.startProjectIntake({
    ...
    mode: 'internal',
    title,
    rawRequest: 'Produce a concise bounded explanation of why autonomous execution must preserve independent verification and human authority.',
    requestedSolution: 'Provider-neutral free model execution'
  });
  snapshot = store.saveDiscoveryResponses({
    ...
    responses: [
      ...
      { questionKey: 'desired_outcome', responseState: 'answered', answerText: 'A real free route produces bounded evidence, an independent real route verifies it, and Workflow OS preserves authority.' },
      { questionKey: 'primary_users', responseState: 'answered', answerText: 'Workflow OS operator' },
      { questionKey: 'constraints', responseState: 'answered', answerText: 'Synthetic data only; zero paid spend; no production/destructive side effects.' },
      { questionKey: 'success', responseState: 'answered', answerText: 'Execution and independent verification pass while cost records and spend envelopes remain zero.' }
    ]
  });
...
function bestWorkerRoute(routes, providerKey) {
  return routes
    .filter((route) => route.provider_key === providerKey && route.enabled === 1 && route.quality_score > 0)
